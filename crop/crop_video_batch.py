...
def crop_video(input_video_path: str, bounds_tuple: Tuple[int, int, int, int], output_video_path: str) -> None:
    ...
    if not os.path.isfile(output_video_path):
        ...
        stream = ffmpeg.output(audio, stream, output_video_path)
...
if __name__ == '__main__':
    ...
    logging.basicConfig(filename='log_crop_video_batch{}.log'.format(datetime.datetime.now().strftime('%H_%M_%d_%m_%Y')),
                        level=logging.DEBUG)
    ...
    parser.add_argument('--ov_dir',
                        ...
                        required=True)
    parser.add_argument('--ov_format',
                        help='output video format. Ex mp4 or m4v',
                        required=True)
